...
//	while((atk_8266_consta_check()-'2'));   //获得IP
	LCD_Clear(WHITE);
	POINT_COLOR=RED;
	Show_Str(30,10,200,16,"ATK-ESP AP+STA模式测试",16); 
...
PRESTA:
	netpro=atk_8266_netpro_sel(50,10,(u8*)ATK_ESP8266_CWMODE_TBL[0]);	//AP+STA模式网络模式选择
	if(netpro&0X02)   //STA UDP
	{
				LCD_Clear(WHITE);
				POINT_COLOR=RED;
				Show_Str_Mid(0,10,"ATK-ESP WIFI-STA测试",16,240); 
	...
	else     //TCP
	{
		if(netpro&0X01)     //STA TCP Client  
		{
			...
			while(atk_8266_send_cmd(p,"OK",200))
			{
				...
				Show_Str(30,60,200,12,"ATK-ESP 连接UDP失败",12); //连接失败	 
				key=KEY_Scan(0);
	...
	LCD_Clear(WHITE);
	POINT_COLOR=RED;
	Show_Str_Mid(0,10,"ATK-ESP WIFI-STA+AP 测试",16,240);
	Show_Str(15,30,200,16,"正在配置ATK-ESP模块,请稍等...",12);			
	LCD_Fill(15,30,239,50+12,WHITE);			//清除之前的显示
	Show_Str_Mid(0,30,"WK_UP:退出 KEY0:ID0发送 KEY1:ID1发送",12,240);
	LCD_Fill(15,60,239,80+12,WHITE);
	atk_8266_get_ip(15,45);//STA+AP模式,获取 IP,并显示	
	
	
	Show_Str(15,75,200,12,"连接状态:",12); 		//连接状态
	Show_Str(15,90,200,12,"STA模式:",12); 		//STA连接状态
	Show_Str(120+15,90,200,12,"AP模式:",12); 		//AP连接状态
	Show_Str(15,105,200,12,"发送数据:",12); 	//发送数据
	Show_Str(15,120,200,12,"接收数据:",12);	//接收数据
	atk_8266_wificonf_show(15,170,"请设置路由器无线参数为:",(u8*)wifista_ssid,(u8*)wifista_encryption,(u8*)wifista_password);
	POINT_COLOR=BLUE;		
	Show_Str(48+15,90,200,12,(u8*)ATK_ESP8266_WORKMODE_TBL[netpro&0X03],12); 		//STA连接状态
	Show_Str(162+15,90,200,12,(u8*)ATK_ESP8266_WORKMODE_TBL[netpro>>4],12); 		//AP连接状态
	USART2_RX_STA=0;
	while(1)
	{
		key=KEY_Scan(0);
		...
		else if(key==KEY0_PRES)	//KEY0 向ID0发送数据 
		{
			  sprintf((char*)p,"ATK-8266模块ID0发数据%02d\r\n",t/10);//测试数据
				Show_Str(15+54,105,200,12,p,12);
	 ...
	 if(timex)timex--;
 	 if(timex==1)LCD_Fill(30+54,105,239,122,WHITE);
	 t++;
	 delay_ms(10);
	...
	{ 
		...
		LCD_Fill(15+54,120,239,152,WHITE);
		POINT_COLOR=BRED;
		Show_Str(15+54,120,156,12,p,12); 			//显示接收到的数据长度
		POINT_COLOR=BLUE;
		LCD_Fill(15,135,239,319,WHITE);
		Show_Str(15,135,180,190,USART2_RX_BUF,12);//显示接收到的数据  
		USART2_RX_STA=0;
	...
	{
		LCD_Fill(15+54,105,239,138,WHITE);
//		LCD_Fill(15+54,75,120,110,WHITE);
		constate=atk_8266_consta_check();//得到连接状态
		if(constate=='+')Show_Str(15+54,75,200,12,"连接成功",12);  //连接状态
		else Show_Str(15+54,75,200,12,"连接失败",12); 	 
		t=0;
	}
	if((t%20)==0)
