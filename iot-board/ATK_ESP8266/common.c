...
const u8* wifista_ssid="bryan_ducourt";			//路由器SSID号
const u8* wifista_encryption="wpawpa2_aes";	//wpa/wpa2 aes加密方式
const u8* wifista_password="0x44 0x59 0x43"; 	//连接密码
...
{		  
	...
	if(keyx>15)return;
	if(sta)LCD_Fill(x+j*80+1,y+i*28+1,x+j*80+78,y+i*28+26,CYAN);
...
u8 atk_8266_get_keynum(u16 x,u16 y)
{
	...
	u8 key=0;
	key = KEY_Scan(0); 		 
	if(key == KEY0_PRES)			//触摸屏被按下
	{	
		if(key_x>15)
		{
			atk_8266_key_staset(0,100,key_x-2,0);
			key_x=0;
		}	
		key_x+=1;
		atk_8266_key_staset(0,100,key_x-1,1);
		atk_8266_key_staset(0,100,key_x-2,0);
		
	}
	if(key == KEY2_PRES)			//触摸屏被按下
	{	
		key_x-=1;
		atk_8266_key_staset(0,100,key_x-1,1);
		atk_8266_key_staset(0,100,key_x,0);
...
void atk_8266_get_ip(u8 x,u8 y)
{
		...
		p1=mymalloc(sramx,32);							//申请32字节内存
		p2=mymalloc(sramx,32);							//申请32字节内存
	  ipbuf=mymalloc(sramx,32);							//申请32字节内存
		buf=mymalloc(sramx,32);							//申请32字节内存
		if(atk_8266_send_cmd("AT+CIFSR","OK",50))//获取WAN IP地址失败
		{ 
			*ipbuf=0;
		}
		...
		{		
			...
			*p1=0;
			...
			Show_Str(x,y,200,12,buf,12);				//显示AP模式的IP地址和端口
			p=(u8*)strstr((const char*)(p2+1),"STAIP,\"");
			p1=(u8*)strstr((const char*)(p+7),"\"");
			*p1=0;
			ipbuf=p+7;
			sprintf((char*)buf,"STA IP:%s 端口:%s",ipbuf,(u8*)portnum);
			Show_Str(x,y+15,200,12,buf,12);				//显示STA模式的IP地址和端口
			myfree(sramx,p);		//释放内存
			myfree(sramx,p1);		//释放内存
			myfree(sramx,p2);		//释放内存
			myfree(sramx,ipbuf);		//释放内存
...
void atk_8266_msg_show(u16 x,u16 y,u8 wanip)
{
	...
	if(wanip==0)//全更新
	{
		...
		*p1=0;
		Show_Str(x,y,240,16,"固件版本:",16);
		Show_Str(x+72,y,240,16,p+12,16);
		atk_8266_send_cmd("AT+CWMODE?","+CWMODE:",20);	//获取网络模式
		p=atk_8266_check_cmd(":");
		Show_Str(x,y+16,240,16,"网络模式:",16);
		Show_Str(x+72,y+16,240,16,(u8*)ATK_ESP8266_CWMODE_TBL[*(p+1)-'1'],16);
  	atk_8266_send_cmd("AT+CWSAP?","+CWSAP:",20);	//获取wifi配置
		p=atk_8266_check_cmd("\"");
		p1=(u8*)strstr((const char*)(p+1),"\"");
		p2=p1;
		*p1=0;
		Show_Str(x,y+32,240,16,"SSID号:",16);Show_Str(x+56,y+32,240,16,p+1,16);
		p=(u8*)strstr((const char*)(p2+1),"\"");
		p1=(u8*)strstr((const char*)(p+1),"\"");
		p2=p1;
		*p1=0;	
		Show_Str(x,y+48,240,16,"密码:",16);Show_Str(x+40,y+48,240,16,p+1,16);
		p=(u8*)strstr((const char*)(p2+1),",");
		p1=(u8*)strstr((const char*)(p+1),",");
		*p1=0;
		Show_Str(x,y+64,240,16,"通道号:",16);Show_Str(x+56,y+64,240,16,p+1,16);
		Show_Str(x,y+80,240,16,"加密方式:",16);Show_Str(x+72,y+80,240,16,(u8*)ATK_ESP8266_ECN_TBL[*(p1+1)-'0'],16);
	}
	myfree(sramx,p);		//释放内存 
	myfree(sramx,p1);		//释放内存 
	myfree(sramx,p2);		//释放内存 
...
{ 
	POINT_COLOR=RED;
	Show_Str(x,y,240,12,rmd,12);//显示提示信息 
	Show_Str(x,y+20,240,12,"SSID:",12);
	Show_Str(x,y+36,240,12,"加密方式:",12);
	Show_Str(x,y+52,240,12,"密码:",12); 
	POINT_COLOR=BLUE;
	Show_Str(x+30,y+20,240,12,ssid,12);
	Show_Str(x+54,y+36,240,12,encryption,12);
	Show_Str(x+30,y+52,240,12,password,12); 	  
}
//工作模式选择
//返回值:
//0,TCP服务器
...
u8 atk_8266_netpro_sel(u16 x,u16 y,u8* name) 
{
	...
	POINT_COLOR=RED;
	u8 sramx = 0;
	p=mymalloc(sramx,50);//申请50个字节的内存
	sprintf((char*)p,"%s 工作模式选择",name);
	Show_Str_Mid(0,y,p,16,240); 				    	 
	Show_Str(x,y+30,200,16,"KEY0:下一个",16); 				    	 
	Show_Str(x,y+50,200,16,"KEY1:上一个",16);				    	 
	Show_Str(x,y+70,200,16,"WK_UP:确定",16); 
	Show_Str(x,y+100,200,16,"请选择:",16); 
	POINT_COLOR=BLUE;
 	Show_Str(x+16,y+120,200,16,"TCP服务器",16); 				    	 
	Show_Str(x+16,y+140,200,16,"TCP客户端",16);				    	 
	Show_Str(x+16,y+160,200,16,"UDP模式",16); 
	POINT_COLOR=RED;
	Show_Str(x,y+120,200,16,"→",16); 
	while(1)
	{
		key=KEY_Scan(0); 
		if(key)
		{
			...
			Show_Str(x,y+120+netpro*20,200,16,"  ",16);//清空之前的显示
			...
			Show_Str(x,y+120+netpro*20,200,16,"→",16);//指向新条目
...
u8 atk_8266_mode_cofig(u8 netpro)
{
	...
	u8 key;
	u8 sramx = 0;
	p=mymalloc(sramx,32);//申请32个字节的内存
PRESTA:		
	netpro|=(atk_8266_netpro_sel(50,10,(u8*)ATK_ESP8266_CWMODE_TBL[1]))<<4;	//网络模式选择
	if(netpro&0X20)
	{
		LCD_Clear(WHITE);
		...
		while(atk_8266_send_cmd(p,"OK",200))
			{
				...
				Show_Str(30,60,200,12,"ATK-ESP 连接TCP SERVER失败",12); //连接失败	 
				key=KEY_Scan(0);
...
u8 atk_8266_ip_set(u8* title,u8* mode,u8* port,u8* ip) 
{
	...
	LCD_Clear(WHITE);  
	POINT_COLOR=RED; 
	Show_Str_Mid(0,10,title,16,240);		//显示标题	 	 	
	Show_Str(30,30,200,16,"工作模式:",16);	//工作模式显示
	Show_Str(30,50,200,16,"IP地址:",16);	//IP地址可以键盘设置
	Show_Str(30,70,200,16,"端口:",16);	//端口号
	kbd_fn_tbl[0]="连接";
	kbd_fn_tbl[1]="返回"; 
	atk_8266_load_keyboard(0,100);			//显示键盘 
	POINT_COLOR=BLUE;
 	Show_Str(30+72,30,200,16,mode,16);	//显示工作模式	
 	Show_Str(30+40,70,200,16,port,16);	//显示端口 	
	ip[0]=0; 		
	while(1)
	{ 
		key=atk_8266_get_keynum(0,100);
		if(key)
		{   
			if(key<12)
			{
			...
			ip[iplen]=0; 
			LCD_Fill(30+56,50,239,40+16,WHITE);
			Show_Str(30+56,50,200,16,ip,16);			//显示IP地址 	
		} 
		timex++;
		if(timex==20)
		{
...
{ 
	...
	Show_Str(x,y+25,200,16,"请选择:",16); 				    	 
	Show_Str(x,y+45,200,16,"KEY0:WIFI STA+AP",16); 				    	 
	Show_Str(x,y+65,200,16,"KEY1:WIFI STA",16);				    	 
	Show_Str(x,y+85,200,16,"WK_UP:WIFI AP",16); 
 	atk_8266_msg_show(x,y+110,0);
}
//ATK-ESP8266模块测试主函数
...
//	u16 rlen=0;
	...
	u8 timex;
	POINT_COLOR=RED;
	Show_Str_Mid(0,10,"ATK-ESP8266 WIFI模块测试",16,240); 
	while(atk_8266_send_cmd("AT","OK",20))//检查WIFI模块是否在线
	{
		atk_8266_quit_trans();//退出透传
		atk_8266_send_cmd("AT+CIPMODE=0","OK",200);  //关闭透传模式	
		Show_Str(40,35,200,16,"未检测到模块!!!",16);
		delay_ms(800);
		LCD_Fill(40,35,200,55+16,WHITE);
		Show_Str(40,35,200,16,"尝试连接模块...",16); 
	} 
		while(atk_8266_send_cmd("ATE0","OK",20));//关闭回显
		atk_8266_mtest_ui(32,10);
	while(1)
	{
		delay_ms(10); 
		...
		if(key)
		{
			...
			{
				case 1://KEY0
					Show_Str_Mid(0,10,"ATK-ESP WIFI-AP+STA 测试",16,240);
					Show_Str_Mid(0,30,"正在配置ATK-ESP8266模块，请稍等...",12,240);
