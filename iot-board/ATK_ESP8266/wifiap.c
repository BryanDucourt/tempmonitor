...
PRESTA:
	...
	if(netpro&0X02)   //UDP
	{
				LCD_Clear(WHITE);
				POINT_COLOR=RED;
				Show_Str_Mid(0,10,"ATK-ESP WIFI-AP 测试",16,240); 
	...
	else     //TCP
	{
		if(netpro&0X01)     //TCP Client    透传模式测试
		{
			...
			Show_Str(30,30,200,16,"配置ATK-ESP模块成功!",12);
			delay_ms(200);
			...
			atk_8266_wificonf_show(30,160,"请用设备连接WIFI热点:",(u8*)wifiap_ssid,(u8*)wifiap_encryption,(u8*)wifiap_password);
			POINT_COLOR=BLUE;
	...
	if(timex)timex--;
	if(timex==1)LCD_Fill(30+54,180,239,112,WHITE);
	t++;
	delay_ms(5);
